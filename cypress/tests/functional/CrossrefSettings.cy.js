 ...
 * integration, and the first test enables the plugin when it is off. Nothing
 * else is saved: the agency is only chosen on the form.
 */
...
describe('Crossref plugin for OMP', function() {
	...
	it('Enables the plugin', function() {
		...
		openPluginsTab();
		enablePlugin('crossrefplugin');
	});

	it('Offers Crossref as the registration agency, with no untranslated text', function() {
		login(adminUser, adminPassword);
		cy.visit(pageUrl('management/settings/distribution') + '?reload=' + Date.now() + '#dois/doisRegistration');
