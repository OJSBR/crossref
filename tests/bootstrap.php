...
/**
 * @file plugins/generic/crossref/tests/bootstrap.php
 *
 ...
 * plugins/generic/crossref, the OMP 3.4 installation around it is bootstrapped so the
...
if (!class_exists('\PKP\plugins\GenericPlugin')) {
    ...
    if (!is_file($root . '/lib/pkp/includes/bootstrap.php')) {
        fwrite(STDERR, "The plugin must be installed in plugins/generic/crossref of an OMP 3.4 installation to run the suite.\n");
        exit(2);
    }
    chdir($root);
...
require_once dirname(__DIR__) . '/CrossrefExportPlugin.php';
require_once dirname(__DIR__) . '/CrossrefPlugin.php';
